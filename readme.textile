...
** Drawback of Wordpress Detection Mode: manual cropping in Wordpress’ Image Editor destroys this ability.
...
But note the drawback: when you edit an image (cropping, scaling, flipping) using the Worpdress Image Editor, you “destroy” this ability: Worpdress then saves another file by the naming convention of **{filename}-{hash}-{width}x{height}** (for example: atestpic-e34389845023-200x200.jpg). With this, there is no “big original” file any more and no information about what was done through editing (where did you crop? did you flip?). Wordpress Detection Mode then simply falls back to the normal file requested – skipping the “re-retinification”.

Maybe in future improvements and when the project “Wordpress Integration for AI(üe)” is going to start, there might be a chance to hook directly in Wordpress Image Editor end prepare this file for us. But I can’t make promises. 
