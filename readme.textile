...
h1. Adaptive Images (über-extended) v2.0
...
* **Wordpress Detection Mode:** “Re-retinize” Wordpress images – serve the pre-scaled Wordpress images according to pixel density
...
But note a small **drawback:** when you edit an image (cropping, scaling, flipping) using the built-in Worpdress Image Editor, **you must apply the editing to all image sizes** (means: no extra cropping for the thumbnail!) and you **need to regenerate all image sizes** afterwards – this way you keep all contents consistent and this is the only way the algorithm can perform its special re-resizing. Reason: there is no information about “where did you crop?”, “where did you flip?” available on a file level any more; so there’s no way to determine what part of the original image needs to be magnified/“retinified”. In case something went wrong and an original picture for "retinification” cannot be fount, Wordpress Detection Mode then simply falls back to the normal file requested – skipping the “retinification” process.
